...
- Схема лежит в проекте по пути: [./task2/arch_sprint1.drawio]()

## Архитектура была изменена в соответствии с Domain-Driven Design


- Анализ Бизнес доменов следующий:
  - Фронтенд разделили на каждую роль клиента. Портал электронной площадки изменили на микрофронтенды по патерну BFF(соотвествие выделили цветом)
  - Все запросы будут проходить через Api Gateway.
  - Все микросервисы необходимо настроить в соотвествии с патерном Хореография (каждый микросервис следит за компенсирующими действиями).
  - Внешний сервис Biling будет работать асинхронно и при неудаче возвращать статус по каждой транзакции в kafka для микросервиса Streams.
